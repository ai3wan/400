...
async def get_components_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        # Топ-10 поставщиков по числу компонентов
        cursor.execute("""
        ...
        # Time series: количество компонентов по месяцам создания
        cursor.execute("""
            SELECT
        ...
        meta = {
            "generated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"),
            "note": "Aggregations for component table"
        }
